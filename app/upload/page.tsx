...
import { ArrowLeft, Upload, Music, Play, Pause, X, FileAudio, AlertCircle } from "lucide-react"
  ...
  const validateForm = (): boolean => {
    ...
    if (!formData.file) {
      ...
      if (!allowedTypes.includes(formData.file.type)) {
        newErrors.file = "Please select a valid audio file (MP3, WAV, M4A, AAC)"
      ...
      <main className="container mx-auto px-4 py-8 max-w-2xl">
        <Card className="bg-white/10 backdrop-blur-sm border-white/20">
          <CardHeader>
            ...
            <p className="text-gray-300 text-center">Share your latest creation with the BARS community</p>
          </CardHeader>
          <CardContent>
            <form onSubmit={handleSubmit} className="space-y-6">
              {/* File Upload */}
              <div className="space-y-2">
                <Label className="text-white text-lg font-semibold">Audio File</Label>
                <div
                  className={`border-2 border-dashed rounded-lg p-8 text-center transition-all duration-300 ${
                    isDragOver ? "border-pink-400 bg-pink-400/10" : "border-white/30 hover:border-white/50"
                  }`}
                ...
                >
                  {formData.file ? (
                    <div className="space-y-4">
                      <div className="flex items-center justify-center space-x-3">
                        ...
                        <div className="text-left">
                          ...
                          <p className="text-gray-400 text-sm">{formatFileSize(formData.file.size)}</p>
                        </div>
                      </div>
                      ...
                      {audioPreview && (
                        <div className="flex items-center justify-center space-x-4">
                          ...
                          <span className="text-gray-300 text-sm">Preview your beat</span>
                        </div>
                      )}
                    ...
                    <div className="space-y-4">
                      ...
                      <p className="text-gray-500 text-xs">Supported formats: MP3, WAV, M4A, AAC (Max 50MB)</p>
                    </div>
                  )}
                </div>
                <input
